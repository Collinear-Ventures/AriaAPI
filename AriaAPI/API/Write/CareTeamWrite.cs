...
{
    /// <summary>
    /// Provides update and upsert operations for FHIR <see cref="CareTeam"/> resources.
    /// </summary>
    /// <remarks>
    /// All methods accept a pre-constructed <see cref="CareTeam"/> resource.
    /// Patient-centric overloads additionally accept a patient identifier (MRN or FHIR
    /// <c>system|value</c> token), resolve it to a FHIR logical Patient ID, and set
    /// <see cref="CareTeam.Subject"/> before writing if not already populated.
    /// PHI is never logged in plain text — identifiers are masked via <see cref="PhiMask"/>.
    /// </remarks>
    public static class CareTeamWrite
    {
        // ── Shape 1: resource-centric ──────────────────────────────────────────

        /// <summary>
        /// Updates an existing <see cref="CareTeam"/> resource on the FHIR server using its logical ID.
        /// </summary>
        /// <param name="configurator">FHIR client configurator.</param>
        /// <param name="resource">
        /// The fully constructed <see cref="CareTeam"/> to update.
        /// <see cref="Resource.Id"/> must be set to the existing server-assigned logical ID.
        /// </param>
        /// <param name="logger">Logger instance.</param>
        /// <param name="versionAware">
        /// When <see langword="true"/> (default), sends an <c>If-Match</c> ETag header for optimistic concurrency.
        /// </param>
        /// <param name="ct">Cancellation token.</param>
        /// <returns>The updated <see cref="CareTeam"/> as returned by the server.</returns>
        ...
        public static async Task<CareTeam> UpdateAsync(
            ...
            CancellationToken ct = default)
        {
            ...
            var client = configurator.ForResource<CareTeam>(ct);
            var updated = await client.UpdateAsync(resource, versionAware).ConfigureAwait(false);

            if (updated is null)
                throw new InvalidOperationException(
                    $"FHIR server returned null for CareTeam update (id: {PhiMask.Mask(resource.Id)}).");
            logger.LogInformation("CareTeam updated with id: {Id}", PhiMask.Mask(updated.Id));
            return updated;
        }

        /// <summary>
        /// Conditionally creates or updates a <see cref="CareTeam"/> resource using a search identifier
        /// (PUT with search criteria — creates if no match, updates if exactly one match).
        /// </summary>
        /// <param name="configurator">FHIR client configurator.</param>
        /// <param name="resource">The fully constructed <see cref="CareTeam"/> to upsert.</param>
        /// <param name="identifier">
        /// Conditional PUT search criteria as a raw query string, e.g. <c>"identifier=urn:aria:careteam|ct-001"</c>.
        /// Must match zero or one existing resources; the server returns 412 if multiple match.
        /// </param>
        /// <param name="logger">Logger instance.</param>
        /// <param name="ct">Cancellation token.</param>
        /// <returns>The created or updated <see cref="CareTeam"/> as returned by the server.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="configurator"/> or <paramref name="resource"/> is <see langword="null"/>.</exception>
        /// <exception cref="ArgumentException">Thrown when <paramref name="identifier"/> is null or whitespace.</exception>
        public static async Task<CareTeam> UpsertAsync(
            ClientConfigurator configurator,
            CareTeam resource,
            string identifier,
            ILogger logger,
            CancellationToken ct = default)
        {
            ...
            if (!identifier.Contains('='))
                throw new ArgumentException(
                    "identifier must be in 'param=value' format (e.g. \"identifier=urn:aria:careteam|ct-001\").",
                    nameof(identifier));
            ...
            if (upserted is null)
                throw new InvalidOperationException(
                    $"FHIR server returned null for CareTeam upsert (identifier: {PhiMask.Mask(resource.Id)}).");
            logger.LogInformation("CareTeam upserted with id: {Id}", PhiMask.Mask(upserted.Id));
            return upserted;
        }
        ...
        /// <summary>
        /// Resolves a patient by MRN or FHIR identifier, sets <see cref="CareTeam.Subject"/> if not
        /// already populated, then updates the resource using its logical ID.
        /// </summary>
        /// <param name="configurator">FHIR client configurator.</param>
        ...
        public static async Task<CareTeam> UpdateForPatientAsync(
            ClientConfigurator configurator,
            ...
            CancellationToken ct = default)
        {
            ...
            if (resource.Subject is null)
                resource.Subject = new ResourceReference($"Patient/{patient.Id!}");
        ...
        /// <summary>
        /// Resolves a patient by MRN or FHIR identifier, sets <see cref="CareTeam.Subject"/> if not
        /// already populated, then conditionally creates or updates the resource.
        /// </summary>
        /// <param name="configurator">FHIR client configurator.</param>
        /// <param name="patientIdentifier">Patient MRN or FHIR <c>system|value</c> identifier.</param>
        /// <param name="resource">The fully constructed <see cref="CareTeam"/> to upsert.</param>
        /// <param name="identifier">Conditional PUT search criteria, e.g. <c>"identifier=urn:aria:careteam|ct-001"</c>.</param>
        /// <param name="logger">Logger instance.</param>
        /// <param name="ct">Cancellation token.</param>
        /// <returns>The created or updated <see cref="CareTeam"/> as returned by the server.</returns>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="configurator"/> or <paramref name="resource"/> is <see langword="null"/>.</exception>
        ...
        /// <exception cref="InvalidOperationException">Thrown when no patient is found for <paramref name="patientIdentifier"/>.</exception>
        public static async Task<CareTeam> UpsertForPatientAsync(
            ClientConfigurator configurator,
            string patientIdentifier,
            CareTeam resource,
            string identifier,
